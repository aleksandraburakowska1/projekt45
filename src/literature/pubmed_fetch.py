...
def docs_to_rows(docs, query: str, year: int):
    ...
    for d in docs:
        ...
        journal = (
            str(d.get("FullJournalName", "")).strip()
            or str(d.get("Source", "")).strip()
        )
...
def main():
    ...
    for q in queries:
        term = build_pubmed_term(q, args.year)
        ...
        summary_rows.append(
            {"query": str(q).strip(), "year": int(args.year), "count": total_count, "retmax": retmax}
        )
        ...
        rows.extend(docs_to_rows(docs, query=q, year=args.year))
